...
from modules.db.initial import get_document, update_document, update_comment, get_all_comment
...
class Document(tornado.web.RequestHandler):
    ...
    def get(self):
        id_doc = str(self.get_argument("id_doc"))
        self.write(get_document(int(id_doc)))

    def post(self):
        id_doc = str(self.get_argument("id_doc"))
        data = json.loads(self.request.body)
        self.write(update_document(int(id_doc), data))
...
class Comment(tornado.web.RequestHandler):
    ...
    def get(self):
        # id_doc = str(self.get_argument("id"))
        self.write(get_all_comment())

    def post(self):
        data = json.loads(self.request.body)
        id_com = str(self.get_argument("id"))
        if id_com == 'new':
            self.write(update_comment('new', data))
        elif type(id_com) == int:
            self.write(update_comment(int(id_com), data))
...
application = tornado.web.Application([
    ...
    (r"/api/document", Document),
    (r"/api/comment", Comment)
], debug=True)
